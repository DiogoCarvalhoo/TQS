...
public class CarControllerTest {
    ...
    public void whenPostCar_thenCreateCar( ) throws Exception {
        ...
        mockMvc.perform(post("/api/cars").contentType(MediaType.APPLICATION_JSON).content(JsonUtil.toJson(bmw)))
    ...
    public void givenCars_whenGetCars_thenReturnJsonArray() throws Exception {
        ...
        mockMvc.perform(get("/api/cars").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(3))).andExpect(jsonPath("$[0].maker", is(bmw.getMaker()))).andExpect(jsonPath("$[1].maker", is(opel.getMaker())))
